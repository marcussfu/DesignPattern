...
When  to use?
 ...
 客戶端只要統一從Facade這個介面就可以不用管背後的邏輯，一種整理性的模式，亦可以把外觀模式當作重構的起點。
 
 Flyweight
 --------------------------
在大量物件需要創建時，藉由分享的方式避免重覆創建(將已創建的物件記錄起來，並在要使用的時候取出)，可以減少創建不必要的物件。

避免需要大量創建物件的狀況，使物件的使用更有效且能共享物件，儘可能減少記憶體使用量以及分享資訊給相似物件。

FlyweightFactory: 工廠模式的應用，用來建立並管理Flyweight物件，如果物件不存在則建立，存在則返回已建立的物件，可以讓使用這個工廠建立的物件提供給大家共同分享使用。

Flyweight: 所有Concrete的Flyweight物件都需要繼承此介面

對於可以共享的物件，會先在Object Pool中查找，有就回傳，沒有才產生新的
如果系統生成很多物件，且物件本身某些部份不用區分差異，可分出共享元件與外部資源。

優點: 降低物件生成個數以減少記憶體使用，且物件可被集中於一處管理。
缺點: 造成程式架構複雜，且共用共享元件，無法有差異化不同的行為(例: 兩個都是步兵，因為用同一個物件，不知道哪個是哪個)。

Composite
--------------------------
將物件組合成樹狀結構，以表示「部分 - 整體」的層次結構，所有物件都是整體的一部分，而整體是物件的集合。
使用者對整體或單一物件可以使用同樣的Function進行操作，不用在乎要處理的物件是個體物件或組成物件，針對其所定義的一致介面進行操作，意即「讓Client可以用一致的方式看待個體和組成物件」。

應用情境:
軍隊命令下達，將軍下令攻擊，統領的步兵騎兵也要一起進攻，但個別的士兵也可以自行攻擊
